...
from data_app.data_json import load_data, save_data, load_info

from utilities import closing, pseudo
...
menu_line_frame = Frame(menu_frame, bg="#9c97ac", height=1)
...
text_label = Label(menu_stat_frame, text="Nombre projet", font=("Arial", 12), bg=color_menu, fg="#bebebe")
...
liste_button = []
for list_project in info["list_project"]:
    photo_list_project = ImageTk.PhotoImage(
        image=Image.open("Picture/App/" + list_project + ".png").resize((20, 20), Image.LANCZOS)
    )
    
    button_list_project = Button(menu_liste_frame, text=list_project, image=photo_list_project, font=("Arial", 13), fg="#ffffff", bg=color_menu, cursor="hand2", relief="flat", bd=0, highlightthickness=0, highlightbackground="white", activebackground=color_menu, activeforeground="#ffffff", compound="left", padx=10, anchor="w")
    button_list_project.image = photo_list_project
    button_list_project.pack(ipadx=2, ipady=2, side=TOP, fill=X, padx=10, pady=2)
    button_list_project.config(command=lambda project=list_project: button_color_clicked(project))
    liste_button.append(button_list_project)


def button_color_clicked(project):
    if project == "C":
